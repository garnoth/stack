...
/* need to plumb in some interface reference so we can know what our IP and MAC
 * is but until then lets use this
 */
static uint32_t fixme_dst_ip;
char fixme_ip[] = "192.168.0.1"; 
unsigned char fixme_mac[] = {0x31, 0x2C, 0x4A, 0x9B, 0xb4, 0x01};
...
// fixme figure out netdevice propegation (our IP address and reciving device)
void arp_system_init()
{
    // init the head before use
    SLIST_INIT(&head);
    inet_pton(AF_INET, fixme_ip, &fixme_dst_ip);
    fixme_dst_ip = ntohl(fixme_dst_ip);
    printf("After:  %d\n", fixme_dst_ip);
}
...
void arp_recv( struct eth_hdr * eth_header)
{
    ...
    if ((ntohs(arp_header->hwtype) == ARP_HRD_ETH ) && 
        ...
        if (ntohs(arp_header->protype) == ETHERTYPE_IP) { // only process ipv4 arp
            ...
            if (!memcmp(&arp_ipv4->dst_ip, &fixme_dst_ip, ETH_ADDR_LEN)) {
                ...
                if (arp_header->opcode == ARP_REQUEST) {
                    ...
                    arp_send(ntohl(arp_ipv4->src_ip), arp_ipv4->src_mac, ARP_REPLY);
                }
...
void arp_send(uint32_t dst_ip, unsigned char * dst_mac, int opcode)
{
    ...
    memcpy(arp_ipv4->src_mac, &fixme_mac , ETH_ADDR_LEN);
    arp_ipv4->src_ip = htonl(fixme_dst_ip); 
